...
app = FastAPI(title="CryptoVault Enhanced API", version="3.0.0")
...
# Supported Cryptocurrencies (NOW INCLUDING MONERO)
...
class PaymentRequest(BaseModel):
    payment_method: str  # 'bank_transfer', 'card', 'crypto'
    amount: float
    crypto: str
...
# WebSocket Manager (Enhanced for Real-time Chat)
class ConnectionManager:
    ...
    def disconnect_admin(self, websocket: WebSocket):
        if websocket in self.admin_connections:
            ...
            print(f"❌ Admin disconnected. Remaining: {len(self.admin_connections)}")
        ...
        if user_id in self.active_connections:
            ...
            for connection in self.active_connections[user_id]:
                try:
                    ...
                    print(f"✉️ Sent message to user {user_id}")
    ...
    async def send_to_admin(self, message: dict):
        ...
        for connection in self.admin_connections:
            try:
                await connection.send_json(message)
                print(f"✉️ Sent message to admin")
            except Exception as e:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            return {
                ...
                "XMR": data.get("monero", {}).get("usd", 155),  # MONERO ADDED
            }
    except:
        return {
            "BTC": 45000, "ETH": 2500, "USDT": 1, "BNB": 300, 
            "SOL": 100, "XRP": 0.5, "ADA": 0.4, "DOT": 7.5,
            "XMR": 155  # MONERO DEFAULT
        }

MINING_RATES = {
    "BTC": 0.00001, "ETH": 0.0001, "BNB": 0.001, "SOL": 0.01,
    "USDT": 0.1, "XRP": 1.0, "ADA": 1.0, "DOT": 0.1,
    "XMR": 0.005,  # MONERO MINING RATE
}
...
async def startup_db():
    ...
    print("✅ CryptoVault Enhanced API v3.0 Started with Monero Support!")
...
async def register(user: UserRegister):
    ...
    return {"token": token, "user": {"id": user_id, "email": user.email, "full_name": user.full_name, "balances": user_data["balances"]}}
...
async def login(credentials: UserLogin):
    ...
    return {"token": token, "user": {"id": user["id"], "email": user["email"], "full_name": user["full_name"], "kyc_verified": user.get("kyc_verified", False), "kyc_status": user.get("kyc_status", "not_submitted"), "balances": user.get("balances", {})}}
...
async def admin_login(credentials: AdminLogin):
    ...
    return {"token": token, "admin": {"id": admin["id"], "username": admin["username"], "email": admin["email"]}}

# Crypto Prices (Including Monero)
@app.get("/api/crypto/prices")
async def get_crypto_prices():
    prices = await fetch_live_prices()
    return {"prices": prices, "supported_cryptos": SUPPORTED_CRYPTOS, "timestamp": datetime.utcnow().isoformat()}

# Trading (Including Monero)
...
async def start_mining(request: MiningRequest, user: dict = Depends(get_current_user)):
    ...
    return {"success": True, "message": f"Mining {request.crypto} started", "rate": MINING_RATES[request.crypto], "session": mining_session}
...
async def stop_mining(request: MiningRequest, user: dict = Depends(get_current_user)):
    ...
    return {"success": True, "earnings": earnings, "crypto": request.crypto, "duration_hours": hours, "new_balances": new_balances}
...
async def mining_status(user: dict = Depends(get_current_user)):
    ...
    return {"active_sessions": len(sessions), "current_earnings": current_earnings, "sessions": sessions}
...
# ENHANCED CHAT SYSTEM (Real-time bidirectional)
...
async def send_chat_message(
    ...
    # Send to ALL admin connections immediately
    await manager.send_to_admin({
...
# PAYMENT SYSTEM (Bank, Card, Crypto)
...
async def get_my_payments(user: dict = Depends(get_current_user)):
    ...
    return {"payments": serialize_docs(payments), "total": len(payments)}
...
async def get_leaderboard():
    entries = await db.leaderboard.find().sort("rank", 1).to_list(length=None)
...
async def get_all_users(admin: dict = Depends(get_current_admin)):
    ...
    return {"users": serialize_docs(users), "total": len(users)}

@app.post("/api/admin/fund-user")
async def fund_user(request: FundRequest, admin: dict = Depends(get_current_admin)):
    ...
    new_balances = user["balances"].copy()
    new_balances[request.crypto] = new_balances.get(request.crypto, 0) + request.amount
    
    await db.users.update_one({"id": request.user_id}, {"$set": {"balances": new_balances}})
    await manager.send_personal_message({
        "type": "balance_update",
        "crypto": request.crypto,
        "amount": request.amount,
        "new_balance": new_balances[request.crypto]
    }, request.user_id)
    
    return {"success": True, "new_balances": new_balances}
...
async def get_pending_kyc(admin: dict = Depends(get_current_admin)):
    ...
    return {"submissions": serialize_docs(submissions), "total": len(submissions)}
...
async def get_admin_chat_messages(admin: dict = Depends(get_current_admin)):
    ...
    for msg in messages:
        ...
        if user_id in user_chats:
            user_chats[user_id]["messages"].append(msg)
...
async def admin_chat_reply(
    ...
    # Mark admin's view of user messages as read
    await db.chat_messages.update_many(
        {"user_id": user_id, "sender": "user", "read": False},
        {"$set": {"read": True}}
    )
    
    # Send to user immediately via WebSocket
    await manager.send_personal_message({
...
async def verify_payment(verification: PaymentVerification, admin: dict = Depends(get_current_admin)):
    ...
    # If approved, credit user's account
    if verification.status == "approved":
        ...
        if user:
            ...
            await manager.send_personal_message({
                ...
                "message": f"Your payment of {amount} {crypto} has been approved and credited to your account!"
            }, payment["user_id"])
    else:
        ...
        await manager.send_personal_message({
            ...
            "message": f"Your payment has been rejected. Reason: {verification.notes}"
        }, payment["user_id"])
    
    return {
        "success": True,
        "message": f"Payment {verification.status}",
        "payment": {**payment, **update_data}
    }

@app.post("/api/admin/announcement")
async def create_announcement(announcement: Announcement, admin: dict = Depends(get_current_admin)):
    ann_data = {"id": str(uuid.uuid4()), **announcement.dict(), "created_at": datetime.utcnow().isoformat()}
    await db.announcements.insert_one(ann_data)
    await manager.broadcast({"type": "announcement", "announcement": ann_data})
    return {"success": True, "announcement": ann_data}

@app.post("/api/admin/leaderboard-update")
async def update_leaderboard(entry: LeaderboardEntry, admin: dict = Depends(get_current_admin)):
    if entry.crypto_traded not in SUPPORTED_CRYPTOS:
        raise HTTPException(status_code=400, detail="Invalid cryptocurrency")
    await db.leaderboard.update_one({"user_id": entry.user_id}, {"$set": entry.dict()}, upsert=True)
    return {"success": True}

@app.delete("/api/admin/leaderboard-delete/{user_id}")
async def delete_leaderboard_entry(user_id: str, admin: dict = Depends(get_current_admin)):
    await db.leaderboard.delete_one({"user_id": user_id})
    return {"success": True}

@app.post("/api/admin/maintenance")
async def set_maintenance(enabled: bool, message: str = "", admin: dict = Depends(get_current_admin)):
    await db.settings.update_one({"key": "maintenance"}, {"$set": {"enabled": enabled, "message": message}}, upsert=True)
    if enabled:
        await manager.broadcast({"type": "maintenance", "enabled": True, "message": message})
    return {"success": True}
...
async def health_check():
    return {
        "status": "ok",
        "version": "3.0.0",
        "features": "enhanced_with_monero_kyc_restrictions_payment_system",
